...
class DualMomentumStrategy(Strategy):
    ...
    def __init__(self,
        ...
        Args:
            ...
            tickers: List of ticker symbols to trade. If None, defaults to ["AAPL"].
    ...
    def generate_signals(self, market_data: Any) -> List[Signal]:
        ...
        Returns:
            List of Signal objects (BUY/SELL) for each ticker.
        """
        signals = []
        for ticker in self.tickers:
            ...
            if hasattr(market_data, "get_history"):
                prices = market_data.get_history(ticker, self.lookback_days + 1)
            elif isinstance(market_data, dict) and ticker in market_data:
                prices = market_data[ticker]
            
            # Check if we have enough data
            if len(prices) <= self.lookback_days:
                # Not enough data for momentum calculation for this ticker
                continue
